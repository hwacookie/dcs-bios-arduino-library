...
/*			unsigned long now = millis();
			...
			if ((now - lastDebounceTime) >= debounceDelay_)
			{
				...
				{
					Serial.printf("try to send pin %d, state=%d \n", pin_, state);
	...
	public:
		Switch2PosT(const char *msg, char pin, bool reverse = false, unsigned long debounceDelay = 50) : PollingInput(pollIntervalMs)
		{
			msg_ = msg;
			pin_ = pin;
	...
	public:
		SwitchWithCover2PosT(const char *switchMessage, const char *coverMessage, char pin, bool reverse = false, unsigned long debounceDelay = 50) : PollingInput(pollIntervalMs)
		{
...
/*
	...
	public:
		Switch3PosT(const char *msg, char pinA, char pinB, unsigned long debounceDelay = 50) : PollingInput(pollIntervalMs)
		{
			msg_ = msg;
			pinA_ = pinA;
			pinB_ = pinB;
	...
	public:
		SwitchMultiPosT(const char *msg, const byte *pins, char numberOfPins, bool reverse = false) : PollingInput(pollIntervalMs),
																									  lastState_(0)
		{
			msg_ = msg;
			pins_ = pins;
